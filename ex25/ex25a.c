/** WARNGING: This code is fresh and potentially  isn't correct yet */
...
int read_string(char **out_string)
{
	...
	int result = gets(out_string);
	// check(result > 0, "Input error.");
...
int read_scan(const char *fmt, ...)
{
	...
	for (i = 0; fmt[i] != '\0'; i++) {
		if (fmt[i] == '%') {
			...
			switch (fmt[i]) {
				case '\0':
					sentinel("Invalid format, you ended with a %%");
					break;
